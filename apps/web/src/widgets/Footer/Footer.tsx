...
export const Footer = () => {
  ...
  if (isVoid(response) || isVoid(response.data)) {
    return (
      <chakra.footer
        ...
        alignItems="center"
      >
        {(isVoid(response) || isVoid(response.data)) &&
          isVoid(response?.error) && (
            <Text fontSize="xl" fontWeight="medium">
              {locale === "ru"
                ? "Нет данных для отображения футера"
                : "No data for footer rendering"}
            </Text>
          )}
  ...
  const { city, address, contacts, working_time, pages, socials, map_link } =
    response.data;
